  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Bulk Order Requests</h1>
        <span className="text-sm text-gray-500">{total} request{total !== 1 ? 's' : ''}</span>
      </div>

      <div className="flex gap-2 flex-wrap">
        <button onClick={() => { setFilter(''); setPage(1); }} className={`px-3 py-1.5 rounded-lg text-sm ${!filter ? 'bg-[#1B3A6B] text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>All</button>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-12 text-center text-gray-400">No requests found.</div>
      ) : (
            ...
            <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              <button className="w-full flex items-center justify-between p-4 hover:bg-gray-50 text-left" onClick={() => toggle(item.id, item)}>
                <div className="flex items-center gap-3">
                  ...
                  {item.adminNote && <span className="text-xs text-gray-400 italic">[Has note]</span>}
                </div>
              ...
              {expanded === item.id && (
                <div className="border-t border-gray-100 p-4 space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                    <div><p className="text-gray-500">Contact Person</p><p className="font-medium">{item.contactName}</p></div>
                    <div><p className="text-gray-500">Email</p><p className="font-medium">{item.email}</p></div>
                    <div><p className="text-gray-500">Phone</p><p className="font-medium">{item.phone}</p></div>
                    <div><p className="text-gray-500">City</p><p className="font-medium">{item.city}</p></div>
                    <div className="col-span-2"><p className="text-gray-500">Delivery Address</p><p className="font-medium">{item.deliveryAddress}</p></div>
                    {item.note && <div className="col-span-3"><p className="text-gray-500">Customer Note</p><p className="font-medium">{item.note}</p></div>}
                    {item.userName && <div><p className="text-gray-500">Member</p><p className="font-medium">{item.userName} ({item.userEmail})</p></div>}
                  </div>
                      ...
                      <table className="w-full text-sm">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            <th className="text-right px-3 py-2 text-gray-500">Quantity</th>
                            <th className="text-left px-3 py-2 text-gray-500">Note</th>
                          </tr>
                        </thead>
                        <tbody>
                  ...
                  <div className="flex gap-3 items-end">
                    ...
                    <div className="flex-[2]">
                      ...
                      <input value={editNote[item.id] ?? ''} onChange={e => setEditNote(prev => ({ ...prev, [item.id]: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1B3A6B]" placeholder="Internal note..." />
                    </div>
      ...
      {total > 20 && (
        <div className="flex justify-center gap-2">
          <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-2 border border-gray-200 rounded-lg text-sm disabled:opacity-50">Previous</button>
          <span className="px-3 py-2 text-sm text-gray-600">{page} / {Math.ceil(total / 20)}</span>
          <button onClick={() => setPage(p => p + 1)} disabled={page >= Math.ceil(total / 20)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm disabled:opacity-50">Next</button>
        </div>
      )}
    </div>
  );
